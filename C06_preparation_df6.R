...
plot_df6_dist_daystoopen <- (
  ggplot(data=df6_dist_daystoopen %>%
    ...
    geom_bar(stat="identity", fill="#F6D935") +
    theme_minimal() +
...
ggplot(df6_dist_failedbytyp, aes(fill=FAILED, x=TYP_CAMP, y=TOT_EVENTs)) + geom_bar(stat="identity")

### variable NUM_OPENs ###
df6_dist_opens <- df_6_camp_event_clean_final %>%
  mutate(NUM_OPENs = ifelse(NUM_OPENs > 5, "More than 5", NUM_OPENs)) %>%
  group_by(NUM_OPENs) %>%
  summarize(TOT_CLI = n_distinct(ID_CLI)) %>%
  mutate(PERCENT = TOT_CLI/sum(TOT_CLI)) %>%
  arrange(desc(PERCENT))

ggplot(df6_dist_opens, aes(x= NUM_OPENs, y=TOT_CLI)) + geom_bar(stat="identity")


### variable NUM_CLICKs ###
df6_dist_clicks<-df_6_camp_event_clean_final %>%
  mutate(NUM_CLICKs = ifelse(NUM_CLICKs > 4, "More than 4", NUM_CLICKs)) %>%
  group_by(NUM_CLICKs) %>%
  summarize(TOT_CLI = n_distinct(ID_CLI)) %>%
  mutate(PERCENT = TOT_CLI/sum(TOT_CLI)) %>%
  arrange(desc(PERCENT))
ggplot(df6_dist_clicks, aes(x= NUM_CLICKs, y=TOT_CLI)) + geom_bar(stat="identity")
