...
# Most of clients have inserted phone number
...
# Most clients do not registered any kind of job
...
df_2_cli_account_clean %>%
  ...
  arrange(desc(PERCENT))
df_2_cli_account_clean %>%
  summarize(TYP_CLI_ACCOUNT = n_distinct(TYP_CLI_ACCOUNT))
ggplot(df_2_cli_account_clean, aes(x = TYP_CLI_ACCOUNT)) + geom_bar()
